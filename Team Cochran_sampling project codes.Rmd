...
- Sample size for `ever smoked a cigarette` is `r round(p_s[1],0)`, for `ever smoked a marijuana` is `r round(p_s[2],0)` and for `age at smoking cigarette or marijuana` is `r round(m_s,0)`.
...
- The optimum subsample size for `ever smoked a cigarette` is `r round(b_opt_sol[1],0)`, for `ever smoked a marijuana` is `r round(b_opt_sol[2],0)` and for `age at smoking cigarette or marijuana` is `r round(b_opt_sol[3],0)`.
...
- The optimum first stage cluster size for `ever smoked a cigarette` is `r round(a_opt_sol[1],0)`, for `ever smoked a marijuana` is `r round(a_opt_sol[2],0)` and for `age at smoking cigarette or marijuana` is `r round(a_opt_sol[3],0)`.
...
for(i in 1:3){
deff_all[i,] <- 1+(b_opt_sol[i]-1)*roh_new
}
...
- The optimum sample size for the new SRS design will be `r round(n_opt,0)`.

- Still meeting client's precision because cv of the estimates are `r round(cv_new[1],2)`,`r round(cv_new[2],2)`, `r round(cv_new[3],2)`.
...
- can neglect sampling fraction because `r round(f,4)` is fairly small.

### Putting all the information together

```{r}
# results <- read.csv("C:/Users/USER PC/Dropbox/PhD Year 1 Course Materials/Spring 2023 PhD Courses/SURV625 Applied Sampling/SAMPLING PROJECT/Results Summary.csv")
# View(results)
```
...
stratified_prop_frame$n_h[6] <- stratified_prop_frame$n_h[6]-1 #rounding down to match the counts to total n
```
...
stratified_prop_frame<- stratified_prop_frame%>%
  mutate(a_h_exact = a_opt*W_h,
         a_h = round(a_opt*W_h,0),
         a_h_rounded = round(a_opt*W_h,0))
stratified_prop_frame$a_h[1] <- stratified_prop_frame$a_h[1]+1
stratified_prop_frame$a_h[7] <- stratified_prop_frame$a_h[7]-1
```
...
ppes <- school_frame%>%
   mutate(ID = 1:n())%>%
  group_by(Region,grade_level,.add = TRUE)
...
## cumulative MOS (might report the frame to show the cumulative MOS values)
...
library(xlsx)
# write.xlsx(school_frame_PPES, "C:/Users/USER PC/Dropbox/PhD Year 1 Course Materials/Spring 2023 PhD Courses/SURV625 Applied Sampling/SAMPLING PROJECT/PPES_frame.xlsx")
```
...
- One of the selected schools is undersized itself (i.e. having a total student of size 24 which is less than the targeted subsample size for that school). Even for some of the selected schools, the subsequent school on the list was undersized. So, we needed to perform linking, Since the frame is quite large, we linked after selection. The linked frame with the MOS after linking is presented below.

```{r}
school_frame_PPES_linked <- read.xlsx("C:/Users/USER PC/Dropbox/PhD Year 1 Course Materials/Spring 2023 PhD Courses/SURV625 Applied Sampling/SAMPLING PROJECT/PPES_frame_linked.xlsx", sheetName = "Sheet1")
```

### c) Subsampling

- Now that we have our targeted subsample size within each school, we can send officials to the schools to collect the total number of students enrolled from the current register maintained by each school. Or, alternatively, we can refer to a comparatively recent survey that can give us the actual size of the schools selected. 

```{r}
## first stage sampling rate f_a, second stage sampling rate f_b
school_frame_PPES_linked$a_h <- c(stratified_prop_frame$a_h[1],stratified_prop_frame$a_h[2],stratified_prop_frame$a_h[3],stratified_prop_frame$a_h[4],rep(stratified_prop_frame$a_h[5],stratified_prop_frame$a_h[5]),rep(stratified_prop_frame$a_h[6],stratified_prop_frame$a_h[6]),rep(stratified_prop_frame$a_h[7],stratified_prop_frame$a_h[7]),rep(stratified_prop_frame$a_h[8],stratified_prop_frame$a_h[8]),rep(stratified_prop_frame$a_h[9],stratified_prop_frame$a_h[9]))
school_frame_PPES_linked$MOS_sum <- c(stratified_prop_frame$N_h[1],stratified_prop_frame$N_h[2],stratified_prop_frame$N_h[3],stratified_prop_frame$N_h[4],rep(stratified_prop_frame$N_h[5],stratified_prop_frame$a_h[5]),rep(stratified_prop_frame$N_h[6],stratified_prop_frame$a_h[6]),rep(stratified_prop_frame$N_h[7],stratified_prop_frame$a_h[7]),rep(stratified_prop_frame$N_h[8],stratified_prop_frame$a_h[8]),rep(stratified_prop_frame$N_h[9],stratified_prop_frame$a_h[9]))
...
- We can do a systematic sampling within each school with a fractional interval (taking up to 4 decimal places for sampling interval which is inverse of the specified second stage rate) after obtaining the current roster of students and sorting the list by some appropriate auxiliary information available in the list (correlated to the outcome variable of interest, for example, family background information of individual students like SES, family income, neighborhood of residency etc). In case of absence of any relevant auxiliary variable, we can assume that the roster is randomly distributed within each school and then we can draw a simple random sample of students without replacement(ultimate cluster concept). 
...
stratified_prop_frame <- stratified_prop_frame%>%
  mutate(f_h = (a_h*targeted_b_h)/N_h) ## epsem because f_h = f for all h
...
```{r}
## sampling 42 students from Applestamp Middle school; systematic sorted by grade, do teachers matter?
